...
import { JSXAttributeUtils, ExpressionAnalysisUtils, PathNavigationUtils, } from "./utils/shared-utils.js";
...
export class DataItemIdProcessor {
    ...
    process(path) {
        ...
        if (!expression)
            return;
        if (!this.expressionUtils.isIdAccess(expression))
            return;
        const optionalExpr = this.types.isMemberExpression(expression)
            ? this.expressionUtils.createOptionalChainExpression(expression)
            : expression;
        this.attributeUtils.addExpressionAttribute(path, DATA_COLLECTION_ITEM_ID, optionalExpr);
    }
    /**
    ...
    processRootElement(path) {
        ...
        if (this.hasSpreadAttribute(path))
            return;
        ...
        if (this.functionContainsMapCall(fn))
            return;
    ...
    hasSpreadAttribute(path) {
        return path.node.attributes.some((attr) => this.types.isJSXSpreadAttribute(attr));
    }
    functionContainsMapCall(fn) {
        let found = false;
     ...
     */
    buildCollectionItemIdExpression(fn) {
        ...
        if (firstParam.isObjectPattern()) {
            ...
            for (const prop of firstParam.get("properties")) {
                ...
                if (this.types.isStringLiteral(key) &&
            ...
            const newProp = this.types.objectProperty(this.types.stringLiteral(DATA_COLLECTION_ITEM_ID), this.types.identifier(FORWARDING_PARAM_NAME));
            const restIdx = firstParam.node.properties.findIndex((p) => this.types.isRestElement(p));
            if (restIdx === -1) {
        ...
        if (firstParam.isIdentifier()) {
            // props["data-collection-item-id"] — no params modification needed
            return this.types.memberExpression(this.types.identifier(firstParam.node.name), this.types.stringLiteral(DATA_COLLECTION_ITEM_ID), true // computed
            );
        }
        return null;
    }
    findKeyAttribute(path) {
...
//# sourceMappingURL=collection-item-id-processor.js.map